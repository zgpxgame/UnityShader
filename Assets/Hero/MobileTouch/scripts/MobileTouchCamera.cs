...
#pragma warning restore 414
        ...
        private const float rotationFactor = 5.0f;
        ...
        private bool isInvokingAfterSetCrameraPosition;
        ...
        private bool aotoMove;
        private int autoMovePhase;
        public bool AotoMove { get => aotoMove; set => aotoMove = value; }
        private float autoMoveLerpTime;
        private float m_PredictTime;
        private bool _changeCamFarPos = false;//改变了相机的最远距离
        private float _saveFarPos;
        private float _saveZoomMax;
        ...
        public bool CanMoveing
        {
            ...
            set
            {
                if (canMoveing != value)
                {
                    //Debug.LogErrorFormat("change CanMoveing =={0}", value);
                }
                canMoveing = value;
            }
        }
        public void AutoLookAt(Vector3 target, float zoom = 0, float _time = 0, Action handle = null)
        {
            //这里限制了速度
            aotoMove = false;
            ...
            autoMoveLerpTime = 0;
            m_PredictTime = Mathf.Clamp(_time, 0.02f, 20f);

            startZoom = CamZoom;
            endZoom = Mathf.Clamp(zoom, CamZoomMin, CamZoomMax);
            
            startPos = endPos = transform.position;
            if (PerspectiveZoomMode == PerspectiveZoomMode.FIELD_OF_VIEW)
            {
                Vector3 camCenterIntersection = GetIntersectionPoint(GetCamCenterRay());
                endPos = startPos + (target - camCenterIntersection);
            }
            else if (PerspectiveZoomMode == PerspectiveZoomMode.TRANSLATION)
            {
                endPos = target - Transform.forward * endZoom;
            }
            else if (PerspectiveZoomMode == PerspectiveZoomMode.TRANSLATION_Y)
            {
                autoMovePhase = 0;
                Vector3 camCenterIntersection = GetIntersectionPoint(GetCamCenterRay());
                endPos = startPos + (target - camCenterIntersection);
        ...
        public void LookAt(Vector3 target)
        {
            ...
            if (!isInvokingAfterSetCrameraPosition)
            {
                isInvokingAfterSetCrameraPosition = true;
                isInvokingAfterSetCrameraPosition = false;
            }
        }
            ...
            if (keyboardAndMousePlatforms.Contains(Application.platform) && Input.touchCount == 0)
            {
                ...
                if (Mathf.Approximately(mouseScrollDelta, 0) == false)
                {
                    if (isEditorInputRotate == true)
                    {
                        ...
                        {
                            Vector3 rotationAxis = GetRotationAxis();
                            Transform.RotateAround(GetWorldPoint(), rotationAxis, mouseScrollDelta * 100);
                            ComputeCamBoundaries();
                        }
                    }
                    ...
                    else
                    {
                        ...
                        {
                            if (PerspectiveZoomMode == PerspectiveZoomMode.TRANSLATION
                                || PerspectiveZoomMode == PerspectiveZoomMode.TRANSLATION_Y)
                            {
                                editorZoomFactor = 30;
                            }
                            else
        ...
        private void UpdateAutoMove()
        {
            ...
            if (PerspectiveZoomMode == PerspectiveZoomMode.TRANSLATION_Y)
            {
                if (autoMovePhase == 0)
                {
                ...
                else if (autoMovePhase == 1)
                {
                    float zoomLerpTime = 0.3f;
                    if (autoMoveLerpTime < zoomLerpTime)
                    {
                        CamZoom = Mathf.Lerp(startZoom, endZoom, autoMoveLerpTime / zoomLerpTime);
                    }
                    else
                    {
                        CamZoom = endZoom;
        ...
        private bool InputDonwOnUI()
        {
            if (TouchWrapper.TouchCount > 0 && EventSystem.current.IsPointerOverGameObject(TouchWrapper.Touch0.FingerId))
            {
                return true;
            }
...
#pragma warning restore 162
                ...
                if (enableRotation == true)
                {
                    ...
                    {
                        pinchAngleCurrent += pinchUpdateData.pinchAngleDelta * rotationFactor;
                    }
        ...
        public void RotateAroundToDefault()
        {
            var pos = transform.position;
            var center = GetWorldPoint();
            var vec = pos - center;
            rotateAroundAngle = Vector3.SignedAngle(new Vector3(vec.x, 0, vec.z), Vector3.back, Vector3.up);
            rotateSign = Mathf.Sign(rotateAroundAngle);
            rotateAroundAngle = rotateAroundAngle * rotateSign;
            rotateAroundSpeed = rotateAroundAngle / 0.25f;
        }

        private void UpdateRotateToDefault()
        {
            if (rotateAroundAngle > 0)
            {
                float step = rotateAroundSpeed * Time.unscaledDeltaTime;
                if (rotateAroundAngle > step)
                {
                    rotateAroundAngle -= step;
                }
                else
                {
                    step = rotateAroundAngle;
                    rotateAroundAngle = 0;
                }
                transform.RotateAround(GetWorldPoint(), Vector3.up, step * rotateSign);
            }
        }
